...
class Security
{
    ...
    public function createUserGroup($p)
    {
        ...
        if (!empty($id)) {
            trigger_error($this->trans('Group_exists') . ' "' . $p['data']['name'] . '"', E_USER_ERROR);
        }
    ...
    */
    ...
    {
        /* if no user is specified as parameter then calculating for current logged user */
     ...
     * @param  int   $object_id
     * @param  int   $user_id   if not specified - estimating for current logged in user
    ...
    private static function getEstimatedUserAccessForObject($object_id, $user_id = false)
    {
        ...
        if ($r = $res->fetch()) {
            ...
            throw new \Exception(self::trans('Object_not_found'), 1);
        }
        unset($res);
    ...
    public static function getAccessBitForObject($object_id, $access_bit_index, $user_id = false)
    {
        ...
        $dispatcher->dispatch('beforeGetAccessForObject', new BeforeGetAccessForObjectEvent($eventParams));
        ...
        $dispatcher->dispatch('getAccessForObject', new GetAccessForObjectEvent($eventParams));
    ...
    {
        return (Security::isAdmin() || (Security::getAccessBitForObject($object_id, static::$CAN_LIST_FOLDERS, $user_group_id) > 0));
    }
    ...
    public function addObjectAccess($p)
    {
        ...
        $rez['data'][] = $p['data'];
        Cache::get('symfony.container')->get('logger')->error($rez);
    ...
    public function setInheritance($p)
    {
        ...
        if (!empty($r)) {
            ...
            throw new \Exception($this->trans('Object_not_found'));
        }
        ...
        if ($p['inherit']) {
            $dbs->query('DELETE from tree_acl WHERE node_id = $1', $p['id']);
    ...
    public function removeChildPermissions($p)
    {
        ...
        if (!empty($child_ids)) {
            $dbs->query('DELETE FROM tree_acl WHERE node_id in ('.implode(',', $child_ids).')');
            // update inherit flag
            $dbs->query('UPDATE tree SET inherit_acl = 1 WHERE id in ('.implode(',', $child_ids).')');
        }
    ...
    /**
     * Check if userId (or current logged user) is an administrator
     *
     * @param  int     $userId
     * @return boolean
     */
    ...
    /**
     * Check if user_id (or current logged user) can manage users or groups
     *
     * @param  int     $user_id
     * @return boolean
     */
    ...
    /**
     * Check if current logged user is owner for given user id
     *
     * @param  int     $userId
     * @return boolean
     */
    ...
    /**
     * Check if user_id (or current logged user) can add users
     *
     * @param  int     $userId
     * @return boolean
     */
    ...
    /**
     * Check if user_id (or current logged user) can add groups
     *
     * @param  int     $userId
     * @return boolean
     */
    ...
    /**
     * Check if current logged user can edit another user
     *
     * @param  int     $userId
     * @return boolean
     */
